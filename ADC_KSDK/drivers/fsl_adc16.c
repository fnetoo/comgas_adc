 ...
 *
 * Note that the "" has a detailed description.
 * To allow sequential conversions of the ADC to be triggered by internal peripherals, the ADC has more than one
 * group of status and control registers, one for each conversion. The  parameter indicates which group of
 * registers are used, for example,  0 is for Group A registers and  1 is for Group B
 * registers. The
 * s are used in a "ping-pong" approach to control the ADC operation.  At any point, only one of
 * the s is actively controlling ADC conversions. The  0 is used for both software and
 * hardware
 * trigger modes.  1 and greater indicates multiple  registers for
 * use only in hardware trigger mode. See the chip configuration information in the appropriate MCU reference manual for
 * the
 * number of SC1n registers (s) specific to this device.   1 or greater are not used
 * for software trigger operation. Therefore, writing to these s does not initiate a new conversion.
 * Updating the  0 while a different  is actively controlling a conversion is allowed and
 * vice versa. Writing any of the  registers while that specific  is actively controlling a
 * conversion aborts the current conversion.
 *
 * param base          ADC16 peripheral base address.
 * param channelGroup   index.
